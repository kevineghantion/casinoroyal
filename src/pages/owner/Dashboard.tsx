...
import { SeedFundsModal } from '@/components/admin/SeedFundsModal';
import { supabaseAdmin, type RealKPIData } from '@/lib/supabaseAdmin';
...
export default function Dashboard() {
  ...
  const [seedFundsOpen, setSeedFundsOpen] = useState(false);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [kpiData, activityData] = await Promise.all([
        supabaseAdmin.getRealKPIs(),
        supabaseAdmin.getRecentActivity()
      ]);
      setKpis(kpiData);
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      description: `Daily deposits: $${kpis?.dailyDeposits || 0}`,
      icon: TrendingUp,
  ...
  const quickActions = [
    ...
    },
    {
      ...
      icon: DollarSign,
      action: () => setSeedFundsOpen(true),
    },
    {
      title: "Freeze Site",
      description: "Emergency site lock",
      icon: Snowflake,
      action: () => toast({ title: "Warning", description: "Site freeze feature not implemented", variant: "destructive" }),
    ...
    },
    {
      ...
      icon: Activity,
      action: () => navigate('/owner/transactions'),
    },
  ];
