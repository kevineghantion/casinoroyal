...
export default function Users() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const data = await supabaseAdmin.getRealUsers({
        page: currentPage,
        limit: pageSize,
      });
      setUsers(data.users);
  ...
  const handleUserAction = async (actionType: string, actionData?: any) => {
    if (!currentUser) return;
    
    try {
      switch (actionType) {
        case 'updateStatus':
          await supabaseAdmin.updateUserStatus(
            actionData.userId, 
            actionData.status, 
            currentUser.id, 
            actionData.reason
        ...
        case 'updateRole':
          await supabaseAdmin.updateUserRole(
            actionData.userId, 
            actionData.role, 
            currentUser.id, 
            actionData.reason
        ...
        case 'deleteUser':
          await supabaseAdmin.deleteUser(
            actionData.userId, 
            currentUser.id, 
            actionData.reason
          );
        ...
        case 'adjustBalance':
          await supabaseAdmin.adjustUserBalance(
            actionData.userId, 
            actionData.amount, 
            actionData.reason,
            currentUser.id
        ...
        case 'addCredits':
          await supabaseAdmin.addCredits(
            actionData.userId, 
            actionData.amount, 
            actionData.reason,
            currentUser.id
          ...
          toast({
            title: "Success",
            description: `$${actionData.amount.toFixed(2)} credits added successfully`,
          });
          break;
      }
    ...
    },
    {
      ...
      render: (_, user) => (
        <span className="text-sm text-muted-foreground">
          {supabaseAdmin.formatTimeAgo(user.last_login_at)}
        </span>
      ),
    },
    {
